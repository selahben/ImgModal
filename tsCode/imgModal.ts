export function imgModal() {
  ...
  closeModal(document.getElementById("modalCloseBTN") as HTMLElement);
  closeModal(document.getElementById("theModal") as HTMLElement);
  ...
  for (const img of imgs) {
    img.onclick = function (e) {
      ...
      (document.getElementById("modalImg") as HTMLImageElement).src = imgSrc;

      //Changing the Modal's display (opacity is still 0);
      (document.getElementById("theModal") as HTMLElement).style.display =
        "flex";
      ...
      for (let i = 0; i <= 1; i += 0.1) {
        setTimeout(() => {
          (
            document.getElementById("theModal") as HTMLElement
          ).style.opacity = `${i}`;
        }, i * 200);
      }
    };
